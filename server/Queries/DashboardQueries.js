...
average_scores_last_minute AS (
    SELECT
        AVG(helmet_score) AS helmet_score,
        AVG(vest_score) AS vest_score,
        AVG(footwear_score) AS footwear_score,
        AVG(gloves_score) AS gloves_score,
        AVG(scaffolding_score) AS scaffolding_score,
        AVG(guardrail_score) AS guardrail_score,
        AVG(harness_score) AS harness_score
    FROM 
        records
    WHERE 
...
SELECT json_build_object(
    'safetyscore', json_build_object(
        'ppe', json_build_object(
            'helmet', COALESCE((SELECT ROUND(helmet_score) FROM average_scores_last_minute), (SELECT ROUND(helmet_score) FROM latest_scores)),
            'vest', COALESCE((SELECT ROUND(vest_score) FROM average_scores_last_minute), (SELECT ROUND(vest_score) FROM latest_scores)),
            'footwear', COALESCE((SELECT ROUND(footwear_score) FROM average_scores_last_minute), (SELECT ROUND(footwear_score) FROM latest_scores)),
            'gloves', COALESCE((SELECT ROUND(gloves_score) FROM average_scores_last_minute), (SELECT ROUND(gloves_score) FROM latest_scores))
        ),
        'fall', json_build_object(
            'scaffolding', COALESCE((SELECT ROUND(scaffolding_score) FROM average_scores_last_minute), (SELECT ROUND(scaffolding_score) FROM latest_scores)),
            'guardrails', COALESCE((SELECT ROUND(guardrail_score) FROM average_scores_last_minute), (SELECT ROUND(guardrail_score) FROM latest_scores)),
            'harness', COALESCE((SELECT ROUND(harness_score) FROM average_scores_last_minute), (SELECT ROUND(harness_score) FROM latest_scores))
        )
    ),
    'trends', json_build_object(
        'chart', json_build_object(
    ...
    'compliancebreakdown', json_build_object(
        'ppe', json_build_array(
            json_build_object('name', 'helmet', 'value', ROUND(COALESCE((SELECT helmet_score FROM average_scores_last_minute), (SELECT helmet_score FROM latest_scores)))),
            json_build_object('name', 'vest', 'value', ROUND(COALESCE((SELECT vest_score FROM average_scores_last_minute), (SELECT vest_score FROM latest_scores)))),
            json_build_object('name', 'footwear', 'value', ROUND(COALESCE((SELECT footwear_score FROM average_scores_last_minute), (SELECT footwear_score FROM latest_scores)))),
            json_build_object('name', 'gloves', 'value', ROUND(COALESCE((SELECT gloves_score FROM average_scores_last_minute), (SELECT gloves_score FROM latest_scores))))
        ),
        'fall', json_build_array(
            json_build_object('name', 'scaffolding', 'value', ROUND(COALESCE((SELECT scaffolding_score FROM average_scores_last_minute), (SELECT scaffolding_score FROM latest_scores)))),
            json_build_object('name', 'guardrails', 'value', ROUND(COALESCE((SELECT guardrail_score FROM average_scores_last_minute), (SELECT guardrail_score FROM latest_scores)))),
            json_build_object('name', 'harness', 'value', ROUND(COALESCE((SELECT harness_score FROM average_scores_last_minute), (SELECT harness_score FROM latest_scores))))
        )
    ),
    'snapshots', (SELECT json_agg(
        json_build_object(
